...
deletar_repositorio() {
    if [ -d "$REPO_DIR" ]; then
        echo -e "${AMARELO}ATENÇÃO: Esta ação excluirá toda a pasta '$REPO_DIR'!${NC}"
        read -p "Tem certeza? (s/N): " confirma
        if [ "$confirma" = "s" ] || [ "$confirma" = "S" ]; then
            sudo rm -rf "$REPO_DIR"
            echo -e "${VERDE}✔ Repositório deletado.${NC}"
        else
            echo "Ação cancelada."
        fi
    else
        echo -e "${AMARELO}O diretório '$REPO_DIR' não existe.${NC}"
    fi
}
